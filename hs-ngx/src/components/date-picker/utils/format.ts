...
export class DateFormat {
  ...
  static prevDate = function (date, amount = 1) {
  ...
  static getStartDateOfMonth = function (year, month) {
    const result = new Date(year, month, 1);
    const day = result.getDay();

    if (day === 0) {
      return DateFormat.prevDate(result, 7);
    } else {
      return DateFormat.prevDate(result, day);
    }
  };
